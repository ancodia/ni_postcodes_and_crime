...
```{r section1-f_img, fig.align = 'center', out.width = "80%", fig.cap = "limavady.csv", echo = FALSE}
...
```{r section1-g_img, fig.align = 'center', out.width = "80%", fig.cap = "CleanNIPostcodeData.csv", echo = FALSE}
...
```{r section2-a, warning = FALSE, size = "small"}
crime_data <- combine_crime_data("data/NI Crime Data/")
```

```{r section2-a_img, fig.align = 'center', out.width = "80%", fig.cap = "Initial AllNICrimeData.csv", echo = FALSE}
knitr::include_graphics(here::here("images", "all_crime.png"))
```
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}

\newpage
b\) Modifying the crime dataframe so that CrimeID, Reported by, Falls within, LSOA code, LSOA name, last outcome and context are excluded. New structure is saved to a csv file, see below.
```{r section2-b}
...
```
```{r section2-b_img, fig.align = 'center', out.width = "80%", fig.cap = "Modified AllNICrimeData.csv", echo = FALSE}
...
It is evident from the chart that anti-social behaviour (ASBO) is the most prevalent crime category throughout the country by  quite a margin. Recorded instances total close to 180,000 whereas the next highest crime type is Violence and sexual offences (VISO) with around 110,000 occurences.
\noindent\makebox[\linewidth]{\rule{\textwidth}{0.4pt}}
...
Using `str_replace()` from `stringr` remove unwanted text and replace blank values with NA.  
```{r section2-e2}
...
```
A random sample of 5,000 crime records is taken with conditions  included to ignore NA or "No Location" values in the dataset. A seed is set to allow for reproducible results.
```{r section2-f2}
set.seed(100)
...
```
```{r section2-h_img, fig.align = 'center', out.width = "80%", fig.cap = "Sample output of random_crime_sample.csv", echo = FALSE}
